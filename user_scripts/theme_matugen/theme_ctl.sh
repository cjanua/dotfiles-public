...
select_prev_wallpaper() {
    ...
    (( prev_index < 0 )) && prev_index=$(( count - 1 ))

    local selected="${wallpapers[$prev_index]}"
...
select_random_wallpaper() {
    ...
    if (( count == 1 )); then
        ...
        if (( last_index >= 0 && count > 1 )); then
            while (( selected_index == last_index )); do
                selected_index=$(( RANDOM % count ))
            done
        fi
    fi

    local selected="${wallpapers[$selected_index]}"
...
apply_prev_wallpaper() {
    local wallpaper
    wallpaper=$(select_prev_wallpaper) || die "No wallpapers found in ${ACTIVE_THEME_DIR}"
...
Commands:
  set       Update settings and apply changes.
    ...
    next      Cycle to next wallpaper (chronological/natural sort) and apply theme.
    previous  Cycle to previous wallpaper (chronological/natural sort) and apply theme.
    random    Pick a random wallpaper (no immediate repeat) and apply theme.
